...
if __name__ == "__main__":
    ...
    parser.add_argument('dxgenes_file', type=argparse.FileType('r'),
                        help='File containing all relevant diagnostics genes (1 column HGNC geneID)')
    parser.add_argument('alissa_file', type=argparse.FileType('r'),
                        help='HGMD dump file from Alissa (2 columns: 1) HGNC geneID 2)NM_TranscriptID with version)')
    parser.add_argument('alias_file', type=argparse.FileType('r'),
                        help='Alias table (id=Alias gene_id= GeneID in Exoncov)')
                    ...
                    if alias_gene in alissa_genes:
                        if alias_gene in gene_aliases_reversed and len(gene_aliases_reversed[alias_gene]) > 1:
                            for transcript in alissa_genes[alias_gene]:
                                print_dic["Detected_in_Alissa_as_Alias"].append(
                                    [dx_gene, transcript, alias_gene, "WARNING:recursive geneID"]
                                )
                        else:
                            for transcript in alissa_genes[alias_gene]:
