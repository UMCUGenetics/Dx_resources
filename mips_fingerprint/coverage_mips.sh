...
for snp in $snp_positions; do
	...
	if [[ $snp != '2847439' && $snp != '2847910' ]]
		then
			dp_sum=`cat *.vcf | $SnpSift filter POS=$snp | $SnpSift extractFields - GEN[0].DP | awk '{sum+=$1} END {print sum}'`
			dp_avg=`expr $dp_sum / $samples`
			echo $dp_avg
		else
			dp_avg=`cat *.vcf | $SnpSift filter POS=$snp | $SnpSift filter 'isRef(GEN[0])' | $SnpSift extractFields - GEN[0].DP | grep -v "#" | awk '{sum+=$1; if ($1) n++; } END {print sum / n}'`
			echo $dp_avg
	fi
done
