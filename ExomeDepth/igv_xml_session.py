...
def make_igvsession(igv_ed_umcu, igv_ed_hc, bam, vcf_hc, sample_id):
    template_file = Template(open(settings.template_xml).read())
    new_session = str(sample_id) + ".xml"
    igv_ed_hc_log2ratio_test = str(igv_ed_hc) + "_log2ratio_test"
    igv_ed_umcu_log2ratio_test = str(igv_ed_umcu) + "_log2ratio_test"
    ...
    substitute_dic={'session_var' : new_session, 'igv_ed_umcu' : igv_ed_umcu, 
                    ...
                    'sample_id' : sample_id, 'igv_ed_hc_log2ratio_test' : igv_ed_hc_log2ratio_test,
                    'igv_ed_umcu_log2ratio_test' : igv_ed_umcu_log2ratio_test, 
...
if __name__ == "__main__":
    ...
    refdate = opt.callmodel.split("_")[2].split(".")[0]
    ...
    write_file=open("{0}/{1}.xml".format(opt.output,opt.sample_id),"w")
    write_file.write(make_igvsession(igv_ed_umcu,igv_ed_hc,bam_id,vcf_hc,opt.sample_id))
    write_file.close()
