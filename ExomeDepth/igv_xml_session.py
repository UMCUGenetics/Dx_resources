...
def make_family_igvsession(args, statistic, igv_extension, vcf_extension, familyid, child, father, mother, bam):
    ...
    # ID XML variables
    session = "{0}_{1}_{2}_igv.xml".format(familyid, args.runid, statistic)  #Session ID
    snv_vcf_child_id = "SNV/MNV_child:{0}".format(snv_vcf_child.split("/")[-1])  #SNV/MNV track child ID in IGV
    snv_vcf_father_id = "SNV/MNV_father:{0}".format(snv_vcf_father.split("/")[-1])  #SNV/MNV track father ID in IGV
    snv_vcf_mother_id = "SNV/MNV_mother:{0}".format(snv_vcf_mother.split("/")[-1])  #SNV/MNV track mother ID in IGV
...
def make_single(args, igv_extension, vcf_extension, bam):
    for statistic in settings.igv_settings:
        ...
        write_file.write(make_single_igvsession(args, args.sampleid, statistic, igv_extension, vcf_extension, bam))
        write_file.close()

def make_family(args, igv_extension, vcf_extension, bam):
    child, familyid, father, mother = parse_ped(args.ped_file, args.sampleid)
    for statistic in settings.igv_settings:
        write_file = open("{0}/FAM{1}_{2}_{3}_{4}_igv.xml".format(args.output, familyid, args.sampleid, args.runid, statistic), "w")
        write_file.write(make_family_igvsession(args, statistic, igv_extension, vcf_extension, familyid, child, father, mother, bam))
        write_file.close()
...
if __name__ == "__main__":
    ...
    parser_single.add_argument('--fontsize', default=10, type=int, help='fontzise within IGV session for headers [default 12]')
    ...
    parser_family.add_argument('--fontsize', default=10, type=int, help='fontzise within IGV session for headers [default 12]')
    ...
    if args.bam: # Use full path
        ...
        if args.pipeline == "iap": #For analysis based on IAP
            bam = "{1}_dedup.realigned.bam".format(args.sampleid, args.sampleid)  #BAM file
        elif args.pipeline == "nf": #For NF pipeline.
