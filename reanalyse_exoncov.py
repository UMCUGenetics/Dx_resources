...
if __name__ == "__main__":
	...
	group.add_option("-b", default="/hpc/cog_bioinf/data/mapping/diagnostiek/Dx_resources/Tracks/ENSEMBL_UCSC_merged_collapsed_sorted_v2_20bpflank.bed", dest="EXONCALLCOV_BED", help="EXONCALLCOV_BED")
	group.add_option("-g", default="/hpc/cog_bioinf/data/mapping/diagnostiek/Dx_resources/Exoncov/gpanels.txt", dest="EXONCALLCOV_PANEL", help="EXONCALLCOV_PANEL")
	group.add_option("-p", default="/hpc/cog_bioinf/data/mapping/diagnostiek/Dx_resources/Exoncov/Preferred_transcript_list.txt", dest="EXONCALLCOV_PREF", help="EXONCALLCOV_PREF")
	group.add_option("-n", default="/hpc/cog_bioinf/data/mapping/diagnostiek/Dx_resources/Exoncov/NM_ENSEMBL_HGNC.txt", dest="EXONCALLCOV_ENS", help="EXONCALLCOV_ENS")
	...
	group.add_option("-i", default="OFF", dest="use_ini", action="store_false", help="use information from INI file: OFF [default] or ON")
        group.add_option("-f", default="/hpc/cog_bioinf/data/mapping/diagnostiek/Dx_resources/", dest="Dx_resources_folder", metavar="[STRING]", help="Dx_resources_folder")
...
action="python "+str(EXONCALLCOV_PATH)+ " --queue "+str(EXONCALLCOV_QUEUE)+" -a " +str(EXONCALLCOV_TIME)+ " -c "+ str(EXONCALLCOV_MEM) +" -b "+str(EXONCALLCOV_BED)+" -n "+str(EXONCALLCOV_ENS)+" -p "+ str(EXONCALLCOV_PREF)+ " -l "+str(EXONCALLCOV_PANEL)+ " -d "+ str(search_string) 
print action
os.system(action)
...
action2=" sed -i 's/\./\,/g' "+"Exoncov_"+str(Dx_resources_v)+"/*tsv"
action3=" sed -i 's/\./\,/g' "+"Exoncov_"+str(Dx_resources_v)+"/*/*tsv"
os.system(action2)
os.system(action3)
